...
export const bind_calling_codes = (input_id, current=0) => {

    let select_input = document.getElementById(input_id);
    select_input.innerHTML = `<option value="">+CC</option>`;
...
export const return_calling_option_components = (current=0, add_country_name=false) => {

    let options=[];
    options.push(<option value="">+CC</option>);
    ...
    for(let i=0; i<calling_codes.length; i++){
        ...
        options.push(
            <option value={calling_codes[i].code}>
                {calling_codes[i].code} {add_country_name && `- ${calling_codes[i].country}`}
            </option>
        );
    }
